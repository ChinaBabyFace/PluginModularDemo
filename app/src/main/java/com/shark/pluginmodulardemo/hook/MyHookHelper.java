package com.shark.pluginmodulardemo.hook;
...
public class MyHookHelper {
     ...
     *
     */
    public static void inject(Application application, String pluginPath) {
        //拿到本应用的ClassLoader
//        hookActivityResource(application, pluginPath);

        String pluginReleasePath = application.getDir("dex", Context.MODE_PRIVATE).getAbsolutePath();
        DexClassLoader pluginLoader = new DexClassLoader(pluginPath, pluginReleasePath, null, application.getClassLoader());

        PathClassLoader pathLoader = (PathClassLoader) application.getApplicationContext().getClassLoader();
        try {
            //获取宿主pathList
            Object suZhuPathList = getPathList(pathLoader);
            Object chaJianPathList = getPathList(pluginLoader);
...
//        ClassLoader bc = (ClassLoader)baseDexClassLoader;
     ...
     * @param context
     */
    public static void hookActivityResource(Context context, String pluginPath) {
        try {
            //获取ActiivtiyThread类
            ...
            Instrumentation proxy = new MyInstrumentation(baseInstrumentation, context, pluginPath);
            //替换
